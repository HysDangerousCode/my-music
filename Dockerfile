...
ENV   MYSQL_USER_NAME misuc
ENV   MYSQL_PASSWORD hys19990628.
ENV   DATABASE_NAME  my-music
...
# 将构建产物jar包拷贝到运行时目录中
COPY --from=build /app/target/my-music-0.0.1.jar .

# 暴露端口
EXPOSE 80

# 执行启动命令
CMD ["java", "-jar", "/app/my-music-0.0.1.jar"]